...
The connection panel shows a default endpoint and a `+N` control for alternatives such as
loopback, LAN, private-network, or HTTPS endpoints. The default controls the QR code and primary
copy action. You can change it from the expanded list, and T3 Code remembers the endpoint type so
the choice survives ordinary address changes between networks. Without a saved preference, it uses
the built-in LAN endpoint when one is available.
...
You can give a direct or SSH-managed environment a name used only by the current client. Renaming
it later does not change the machine's reported name or restart the connection.
...
Antigravity runs and stores its credentials on the selected environment. Start setup in
**Settings → Providers** on web or desktop, or expand the environment under
**Settings → Environments** on mobile. If Google returns to a `127.0.0.1` page that cannot load,
copy the complete address into T3 Code's return URL field without changing it. Do not paste that
address into a thread or bug report.

## Updating a remote server

When a client and remote server use different versions, T3 Code shows an action in the conversation
and under **Settings → Connections**. It may update and reconnect the server automatically, or ask
you to update the desktop app or run a command on the server. Finish active work first because the
server restarts briefly. See [Updating T3 Code](./updating.md) for the full procedure.
