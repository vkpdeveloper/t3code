...
as unsupported. This works the same way whether the turn came from web, desktop, or mobile. Grok
4.5 can accept an image while interpreting it incorrectly, so T3 Code runs image turns on Grok 4.6
when it is available. Text-only turns keep the selected model. If Grok 4.6 is unavailable, T3 Code
shows an error instead of sending the image to Grok 4.5.
