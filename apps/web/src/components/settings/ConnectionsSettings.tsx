...
export function ConnectionsSettings() {
  ...
  const connectSavedBackendSshTarget = useCallback(
    async (target: DesktopSshEnvironmentTarget, label = "") => {
  ...
  const handleAddSavedBackend = useCallback(async () => {
    if (savedBackendMode === "ssh") {
      ...
      await connectSavedBackendSshTarget(target, savedBackendName);
      return;
    }
  ...
  const handleSelectSshHostSuggestion = useCallback(
    async (target: DesktopDiscoveredSshHost) => {
      ...
      await connectSavedBackendSshTarget(resolved, savedBackendName);
    },
    [connectSavedBackendSshTarget, desktopBridge, isAddingSavedBackend, savedBackendName],
  );
