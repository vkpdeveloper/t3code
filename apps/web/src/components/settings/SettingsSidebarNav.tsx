...
import { isSettingsPathAvailable } from "./settingsSearch";
import {
...
export function SettingsSidebarNav({ pathname }: { pathname: string }) {
  ...
  const activeSettingsPath = availableNavItems.find(
    (item) => pathname === item.to || pathname.startsWith(`${item.to}/`),
  )?.to;
  const observedVisibilityScope = useMemo(() => {
    const path = availableNavItems.find(
      (item) =>
  ...
  }, [availableNavItems, resolvedPathname]);
