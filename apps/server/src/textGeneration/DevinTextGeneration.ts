...
/**
 * Titles, branch names, and commit text through a throwaway `devin acp` session.
 * The session runs in Devin's read-only Ask mode so the model answers without
 * touching the workspace, and is deleted afterwards so it never shows in `devin ls`.
 */
  ...
  const runDevinJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
      ...
      const promptResult = yield* Effect.gen(function* () {
        ...
        const result = yield* runtime.prompt({ prompt: [{ type: "text", text: prompt }] });
        yield* deleteDevinAcpSession(runtime, started.sessionId);
        return result;
      }).pipe(
        ...
        Effect.mapError((cause: EffectAcpErrors.AcpError | TextGenerationError) =>
          isTextGenerationError(cause)
            ? cause
            : new TextGenerationError({ operation, detail: "Devin ACP request failed.", cause }),
        ),
      );
