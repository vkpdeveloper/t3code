...
const make = Effect.gen(function* () {
    ...
    if (thread.session !== null) {
      ...
      yield* rejectStartedThreadModelChangeIfRequired({
        ...
        hasConversationHistory:
          thread.latestTurn !== null || (threadDetail?.messages.length ?? 0) > 0,
      });
    }
    if (
      thread.session !== null &&
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    // The thread shell no longer carries messages; read each queued message
    // from the projection instead.
    const resolvedQueuedMessages: QueuedUsageLimitMessage[] = [];
    let missingQueuedMessage = false;
    for (const messageId of event.payload.queuedMessageIds ?? []) {
      const queuedEntry = yield* projectionSnapshotQuery
        .getTurnStartMessage({ threadId: event.payload.threadId, messageId })
        .pipe(Effect.map(Option.getOrUndefined));
      const message = queuedEntry?.message;
      if (message === undefined || message.role !== "user") {
        missingQueuedMessage = true;
        break;
      }
    ...
    const isFirstUserMessageTurn =
      event.payload.origin !== "usage-limit-auto-resume" && !hasOtherUserMessages;
      ...
      return Ref.get(preflightValidated).pipe(
        Effect.flatMap((validated) =>
          !validated && preserveExistingSessionOnPreflightFailure
            ? appendTurnStartFailure("Provider turn start failed", formatFailureDetail(cause))
