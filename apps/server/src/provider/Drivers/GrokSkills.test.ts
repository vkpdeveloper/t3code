...
describe("discoverGrokSkills", () => {
    ...
    Effect.gen(function* () {
      for (const stdout of ["not json", "null", '{"skills":"nope"}', "{}"]) {
        const error = yield* discoverGrokSkills(
          { binaryPath: "grok" },
          {
            HOME: "/__t3code_missing_grok_skills__",
            GROK_HOME: "/__t3code_missing_grok_skills__/.grok",
          },
        ).pipe(
          Effect.flip,
          Effect.provideService(
  ...
  it.effect("spawns in the configured cwd and rejects a failed probe", () => {
    ...
    return Effect.gen(function* () {
      ...
      const failed = yield* discoverGrokSkills(
        { binaryPath: "grok" },
        {
          HOME: "/__t3code_missing_grok_skills__",
          GROK_HOME: "/__t3code_missing_grok_skills__/.grok",
        ...
        Effect.provide(
          Layer.mergeAll(
            NodeServices.layer,
            Layer.succeed(ChildProcessSpawner.ChildProcessSpawner, makeInspectSpawner(stdout, 1)),
          ),
        ),
      );
