 ...
 *
 * Devin is off by default and instance-only: users add it from Settings →
 * Providers. The model catalog comes from the ACP session's `model` config
 * option during the provider status check (see `DevinProvider`).
 *
...
// `devin update` may prompt before installing, so T3 does not drive it.
