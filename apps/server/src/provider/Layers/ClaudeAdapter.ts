...
/**
 * The CLI's final API error text, when the result carries one. The shared
 * retry policy classifies that concrete message, including provider capacity
 * failures, more accurately than a generic terminal reason.
 */
function claudeApiResultError(result: SDKResultMessage): string | undefined {
  const isApiError = result.is_error === true || result.terminal_reason === "api_error";
...
function resultOutcome(
  ...
  // The CLI's own API error text beats the generic overload notice: the
  // shared retry policy classifies that concrete message, including provider
  // capacity failures, more accurately.
  const structuredError = claudeApiResultError(result) ??
    (isOverloadedResult(result)
      ? "Claude API is overloaded (529). Try again shortly."
      : result.terminal_reason === "api_error"
        ? (failureHint ?? "Claude gave up after repeated API errors.")
...
function claudeUsageLimit(
  ...
  const exactResetAt =
    exactResetAtMs === undefined ? undefined : Option.getOrUndefined(DateTime.make(exactResetAtMs));
  return {
    resetsAt: DateTime.formatIso(
      exactResetAt ?? DateTime.makeUnsafe(fallbackBase + 5 * 60 * 1_000),
    ),
    ...(info.rateLimitType ? { limitType: info.rateLimitType } : {}),
    isEstimated: exactResetAt === undefined,
  };
}
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleAssistantMessage = Effect.fn("handleAssistantMessage")(function* (
    ...
    if (context.turnState) {
      // The CLI can report authentication failure as a synthetic API-error
      // assistant message; retain that evidence for the result fallback
      // before the suppression below skips the message.
      ...
      if ((message as { is_api_error_message?: boolean }).is_api_error_message === true) {
        context.lastAssistantUuid = message.uuid;
        yield* updateResumeCursor(context);
        return;
      }
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    // A rejected usage window always fails the turn and carries the limit so
    // the auto-resume reactor can schedule the retry.
    const status = context.lastRejectedUsageLimit ? "failed" : outcome.status;
    const errorMessage =
  ...
  const handleSdkTelemetryMessage = Effect.fn("handleSdkTelemetryMessage")(function* (
    ...
    if (message.type === "rate_limit_event") {
      ...
      if (context.turnState) {
        ...
        // The auto-resume timer follows the same blocking evidence: a rejected
        // window schedules the retry and a recovery clears it.
        const usageLimit = claudeUsageLimit(rateLimitInfo, base.createdAt);
        if (blocked && usageLimit) {
    ...
    function* (input) {
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        systemPrompt: {
          ...
          append: `${OPERATOR_PROVIDER_INSTRUCTIONS}${T3_CODE_IMAGE_GENERATION_INSTRUCTIONS}${buildRuntimeInstructions({ harness: "Claude Code" })}`,
        },
