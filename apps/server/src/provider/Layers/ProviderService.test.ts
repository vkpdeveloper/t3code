  ...
  const layer = it.layer(
    Layer.mergeAll(
      makeProviderServiceLive().pipe(
        ...
        Layer.provide(input.analyticsLayer ?? analyticsLayer),
        Layer.provide(
          Layer.succeed(
...
routing.layer("ProviderServiceLive routing", (it) => {
  ...
  it.effect("keeps T3 host attachment paths out of Grok image prompts", () =>
    Effect.gen(function* () {
      ...
      const firstTurn = routing.grok.sendTurn.mock.calls[0]?.[0] as {
        input?: string;
        attachments?: unknown;
      };
      ...
      const secondTurn = routing.grok.sendTurn.mock.calls[0]?.[0] as {
        input?: string;
        attachments?: unknown;
      };
      assert.deepStrictEqual(secondTurn.attachments, [attachment]);
      assert.match(secondTurn.input ?? "", /^\[Attached image "screenshot\.png" is saved at: /);
...
describe("agent browser access", () => {
  ...
  const startSessionWith = (
    ...
    options?: { readonly withoutOrchestration?: boolean; readonly enableImageGeneration?: boolean },
  ) =>
    Effect.gen(function* () {
  ...
  // always gets a credential (the pull request and task reference toolkits are
  // never withheld), and `preview` on it is what actually grants or denies the
  // browser tools.
  ...
  it.effect("issues a credential with image generation when the setting is on", () =>
    Effect.gen(function* () {
  ...
  it.effect("issues a credential with preview when agent browser access is on", () =>
    Effect.gen(function* () {
      ...
      const issued = yield* startSessionWith({ browser: true, device: true }, threadId);
