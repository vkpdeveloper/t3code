...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  /**
  * Which optional agent toolsets the credential minted below may drive.
   * Image generation is environment-wide; browser and device access are
   * project-scopable, so their resolution may consult the thread's project.
   *
   * Deny optional toolsets on an unreadable settings file rather than letting
   * the read failure escape: adding `ServerSettingsError` to
   * `ProviderServiceError` would widen a union every caller handles, for a
   * branch that only decides whether optional toolsets are attached. Denying
   * is the safe direction: an explicit "off" silently becoming "on" would
  ...
  const agentAccessSettings = Effect.fn("ProviderService.agentAccessSettings")(
    function* (threadId: ThreadId) {
      ...
      const environment = {
        ...
        imageGeneration: settings.enableImageGeneration,
      };
      ...
      const denied = {
        ...
        imageGeneration: environment.imageGeneration,
      };
    ...
    Effect.catch((cause) =>
      Effect.logWarning(
        ...
        { cause },
      ).pipe(Effect.as({ browser: false, device: false, imageGeneration: false })),
    ),
  );
  ...
  const agentAccessCapabilities = Effect.fn("ProviderService.agentAccessCapabilities")(function* (
    ...
    if (access.imageGeneration) capabilities.add("image-generation");
    return capabilities;
  });
