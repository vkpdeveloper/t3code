...
const resolveThroughExistingAncestor = Effect.fn(
  "AntigravityAdapter.resolveThroughExistingAncestor",
...
const resolveClientFilePath = Effect.fn("AntigravityAdapter.resolveClientFilePath")(
  function* (input: {
    ...
    // Follow symlinks through the closest existing ancestor. New nested paths
    // still inherit the canonical root, including macOS's /var -> /private/var.
    const real = yield* resolveThroughExistingAncestor(input.fileSystem, path, resolved);
      ...
      case "ContentDelta":
        yield* emit(
          makeAcpContentDeltaEvent({
            ...
            streamKind: event._tag === "ThoughtDelta" ? "reasoning_text" : event.streamKind,
            text: event.text,
