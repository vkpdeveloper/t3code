...
function workLogRowsHeight(
  ...
  const imageHeight = activities.reduce(
    (sum, activity) => sum + (activity.generatedImage ? GENERATED_IMAGE_HEIGHT : 0),
    0,
  );
  return (
    activities.length * rowHeight + Math.max(0, activities.length - 1) * WORK_ROW_GAP + imageHeight
  );
}
...
function WorkLogGeneratedImage(props: {
  ...
  return (
    <View className="ml-7 mb-1 h-40 overflow-hidden rounded-lg border border-border">
      {url ? (
        <Pressable
